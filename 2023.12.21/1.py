def pick_resistors(resistance: int) -> dict or None:
    ...
    result = {}
    for series, values in nominals.items():
        min_diff = min(map(lambda x: abs(x - resistance), values))
        closest_values = tuple(filter(lambda x: abs(x - resistance) == min_diff, values))
